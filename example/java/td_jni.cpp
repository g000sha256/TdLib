...
static jint register_native(JavaVM *vm) {
...
#else
  auto td_api_class = td::jni::get_jclass(env, PACKAGE_NAME "/TdlApi");
  ...
  if (tdjni_version != td_api_version) {
    td::jni::set_fatal_error(env, "Mismatched TdlApi.java (" + td_api_version + ") and tdjni shared library (" +
                                      tdjni_version + ") versions");
    return JAVA_VERSION;
  }

  auto client_class = td::jni::get_jclass(env, PACKAGE_NAME "/TdlNative");
  auto object_class = td::jni::get_jclass(env, PACKAGE_NAME "/TdlApi$Object");
  auto function_class = td::jni::get_jclass(env, PACKAGE_NAME "/TdlApi$Function");

#define TD_OBJECT "L" PACKAGE_NAME "/TdlApi$Object;"
#define TD_FUNCTION "L" PACKAGE_NAME "/TdlApi$Function;"
